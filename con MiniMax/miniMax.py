...
class MiniMax:
	...
	def minimax(self, node):
		...
		best_val,best_name = self.max_value(node) # should be root node of tree
		...
		print ("MiniMax:  Utility Value of Root Node: = " + str(best_val)+" with name "+best_name)
	...
	def max_value(self, node):
		...
		for state in successors_states:
			...
			minOfIter,nameOfIter=self.min_value(state)

			if(max_value>minOfIter):
				max_value,max_name =max_value,max_name
			else:
				max_value,max_name=minOfIter,nameOfIter
	...
	def min_value(self, node):
		...
		for state in successor_states:
			'''
			print()
			print("Min value state")
			print(state.Name)
			print()
			'''
			maxOfIter,nameOfIter=self.max_value(state)

			if(min_value<maxOfIter):
				min_value =min_value
				min_name=min_name
			else:
				min_value,min_name =maxOfIter,nameOfIter
